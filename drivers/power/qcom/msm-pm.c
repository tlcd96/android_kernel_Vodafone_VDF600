...
static struct timespec go_to_sleep_ts;
static struct timespec wake_up_ts;
static bool mAsleepAwake = false; // true - sleep, false - awake

void record_app_asleep_awake_time(bool asleep_awake)
{
    struct rtc_time tm;

    if (asleep_awake)
    {
        if (go_to_sleep_ts.tv_sec == 0 || !mAsleepAwake)
        {
            mAsleepAwake = true;
            go_to_sleep_ts = current_kernel_time();
            rtc_time_to_tm(go_to_sleep_ts.tv_sec, &tm);

            printk(KERN_ERR "record_app_asleep_awake_time(): go_to_sleep_ts=%d hour %d min %d sec, awake_time=%ld sec\n",
                tm.tm_hour, tm.tm_min, tm.tm_sec, (go_to_sleep_ts.tv_sec - wake_up_ts.tv_sec));
        }
        else
        {
            printk(KERN_ERR "record_app_asleep_awake_time(): already asleep!\n");
        }
    }
    else
    {
        if (wake_up_ts.tv_sec == 0 || mAsleepAwake)
        {
            mAsleepAwake = false;
            wake_up_ts = current_kernel_time();
            rtc_time_to_tm(wake_up_ts.tv_sec, &tm);

            printk(KERN_ERR "record_app_asleep_awake_time(): wake_up_ts=%d hour %d min %d sec, asleep_time=%ld sec\n",
                tm.tm_hour, tm.tm_min, tm.tm_sec, (wake_up_ts.tv_sec - go_to_sleep_ts.tv_sec));
        }
        else
        {
            printk(KERN_ERR "record_app_asleep_awake_time(): already awake!\n");
        }
    }
}
#endif
	...
	unsigned int cpu, bool from_idle, bool notify_rpm)
{
    ...
    if (collapsed)
    {
        if (cpu == 0 && !from_idle)
        {
            ...
            #ifdef CONFIG_RECORD_APP_ASLEEP_AWAKE_TIME
            record_app_asleep_awake_time(true);
            #endif
        }
        local_fiq_enable();
    }
