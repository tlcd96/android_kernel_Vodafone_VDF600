...
static void iris_clear_buf_events(struct iris_device *radio) 
{ 
	unsigned long flags; 
	enum iris_buf_t buf_type = IRIS_BUF_EVENTS; 
 
	spin_lock_irqsave(&radio->buf_lock[buf_type], flags); 
	kfifo_reset(&radio->data_buf[buf_type]); 
	spin_unlock_irqrestore(&radio->buf_lock[buf_type], flags); 
} 
